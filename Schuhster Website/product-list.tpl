...
{****************************EDITED BY LILIN TO CHANGE HOVER EFFECT TO GET SECOND IMAGE*****************************************}
                                ...
                                <div class="product_image_wrapper">
                                  ...
                                  {**gets second image from database****}

                              {assign var="pImages" value=Product::geImagesByID($product.id_product, 2)}{foreach from=$pImages item=image name=images}
                              <a href="{$product.link|escape:'htmlall':'UTF-8'}" class="product_img_link" title="{$product.name|escape:'htmlall':'UTF-8'}"><img class="product_image"   src="{$link->getImageLink($product.link_rewrite, $image, 'home_default')}" {if $smarty.foreach.images.first}class="current img_{$smarty.foreach.images.index}"{else} class="img_{$smarty.foreach.images.index}" style="display:none;"{/if} alt="{$product.legend|escape:'htmlall':'UTF-8'}" {if isset($homeSize)} width="{$homeSize.width}" height="{$homeSize.height}"{/if}/>
...
{***********************************EDIT ENDS HERE FOR HOVER EFFECT*******************************************}




{*************************************** EDIT OUT BY LILIN TO REMOVE ADD TO CART BUTTON**************************************
