...
{/if}
    ...
    var LoadingText = '{l s='Loading...' js=1}';
	...
	<div id="pb-left-column">
		...
		{if $have_image}
			<span id="view_full_size">
                            <a id="big_pic" class="pb-product-image jqzoom" href="{$link->getImageLink($product->link_rewrite, $cover.id_image, (isset($image_shape) && $image_shape == 'rect_img') ? 'thickr_default' : 'thickbox_default')}" title="{$product->name|escape:'htmlall':'UTF-8'}">
		...
		{else}
			<span id="view_full_size">
				<img class="pb-product-image" src="{$img_prod_dir}{$lang_iso}-default-large_default.jpg" id="bigpic" alt="" title="{$product->name|escape:'htmlall':'UTF-8'}" />
			</span>
		{/if}
		</div>
                        ...
                        <ul id="thumbs_list_frame">
                            {if isset($images)}
                                ...
                                <li id="thumbnail_{$image.id_image}">
                                    <a href="{$link->getImageLink($product->link_rewrite, $imageIds, (isset($image_shape) && $image_shape == 'rect_img') ? 'thickr_default' : 'thickbox_default')}" rel="other-views" class="thickbox{if $smarty.foreach.thumbnails.first} shown{/if}" title="{$image.legend|htmlspecialchars}">
        ...
        <div id="pb-right-wrapper">
            ...
            {if isset($HOOK_RIGHT_COLUMN) && $HOOK_RIGHT_COLUMN}
                        ...
                        <div id="product_price_block">
                            ...
                            <!-- prices -->
                                ...
                                <div class="price" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
                                        ...
                                        {if $priceDisplay == 2}
                                                <br />
                                                <span id="pretaxe_price"><span id="pretaxe_price_display" itemprop="price">{convertPrice price=$product->getPrice(false, $smarty.const.NULL)}</span>&nbsp;{l s='tax excl.'}</span>
                                        {/if}                                        
                                </div>
                                ...
                                <p id="reduction_amount" {if !$product->specificPrice OR $product->specificPrice.reduction_type != 'amount' || $product->specificPrice.reduction|intval == 0} style="display:none"{/if}>
                                    <span id="reduction_amount_display">{if $product->specificPrice AND $product->specificPrice.reduction_type == 'amount' && $product->specificPrice.reduction|intval != 0}-{convertPrice price=$product->specificPrice.reduction|floatval}{/if}</span>
                                </p>
                                ...
                                {if $product->ecotax != 0}
                                        <p class="price-ecotax">{l s='Include'} <span id="ecotax_price_display">{if $priceDisplay == 2}{$ecotax_tax_exc|convertAndFormatPrice}{else}{$ecotax_tax_inc|convertAndFormatPrice}{/if}</span> {l s='for green tax'}
                                                {if $product->specificPrice AND $product->specificPrice.reduction}{l s='(not impacted by the discount)'}{/if}
                                        </p>
                                {/if}
                        ...
                        <div id="product_stock_stat">
                            ...
                            <p id="availability_statut"{if ($product->quantity <= 0 && !$product->available_later && $allow_oosp) OR ($product->quantity > 0 && !$product->available_now) OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>    
                              ...
                              {**********************************************REMOVED BY LILIN*************
                                 ...
                                 <span  style="color: red; font-size: 15px;">Please email: atyourservice@schuhster.com for back order</span>
				...
				<span id="availability_date_value">{dateFormat date=$product->available_date full=false}</span>
                                                    ...
                                                    <fieldset class="attribute_fieldset">
                                                            ...
                                                            {elseif ($group.group_type == 'radio')}
                                                                    {foreach from=$group.attributes key=id_attribute item=group_attribute}
                                                                        ...
                                                                        <input type="radio" class="attribute_radio" name="{$groupName}" value="{$id_attribute}" id="{$groupName}_{$id_attribute}"  {if ($group.default == $id_attribute)} checked="checked"{/if} onclick="findCombination();getProductAttribute();{if $colors|@count > 0}$('#wrapResetImages').show('slow');{/if}">                                                                     

                                                                        <label class="forinput" for="{$groupName}_{$id_attribute}">{$group_attribute|escape:'htmlall':'UTF-8'}</label>
                    ...
                    <div class="quantity_add_cart clearfix">
                        ...
                        <!-- quantity wanted -->
                            ...
                            <div id="set-quantity" {if (!$allow_oosp && $product->quantity <= 0) OR $virtual OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if} >
                                <span class="qty-label" >{l s='Quantity:'}</span>
                            ...
                            {else}
                                <p id="add_to_cart">
                                    <input type="submit" name="Submit" value="{l s='Add to cart'}" class="autumn_add_to_cart" />
                                </p>
                            {/if}
                            ...
                            <div id="minimal_quantity_wanted"{if $product->minimal_quantity <= 1 OR !$product->available_for_order OR $PS_CATALOG_MODE} style="display: none;"{/if}>
...
{if (isset($product) && $product->description) || (isset($features) && $features) || (isset($accessories) && $accessories) || (isset($HOOK_PRODUCT_TAB) && $HOOK_PRODUCT_TAB) || (isset($attachments) && $attachments) || (isset($product) && $product->customizable) || (isset($quantity_discounts) && count($quantity_discounts) > 0)}
	...
	<ul id="more_info_tabs" class="idTabs idTabsShort clearfix">
		...
		{if isset($product) && $product->customizable}<li><a href="#idTab10">{l s='Product customization'}</a></li>{/if}
                {if (isset($quantity_discounts) && count($quantity_discounts) > 0)}<li><a href="#idTabQtyDiscount">{l s='Quantity discount'}</a></li>{/if}
		{$HOOK_PRODUCT_TAB}
	</ul>
        <div id="more_info_shadow">
            ...
            {if isset($features) && $features}
                ...
                {if isset($feature.value)}
                                <li class="product_feature"><span class="product_feature_title">{$feature.name|escape:'htmlall':'UTF-8'}</span><span class="product_feature_data">{$feature.value|escape:'htmlall':'UTF-8'}</span></li>
                {/if}
                    {/foreach}
                    </ul>
            {/if}
                                                        ...
                                                        {if ($accessory.allow_oosp || $accessory.quantity_all_versions > 0 || $accessory.quantity > 0) AND $accessory.available_for_order AND !isset($restricted_country_mode)}
                                                            ...
                                                            <li class="ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description">                                                                            
                                                                    <div class="product_desc">
                                                                            <a href="{$accessoryLink|escape:'htmlall':'UTF-8'}" title="{$accessory.legend|escape:'htmlall':'UTF-8'}" class="product_image"><img src="{$link->getImageLink($accessory.link_rewrite, $accessory.id_image, 'home_default')}" alt="{$accessory.legend|escape:'htmlall':'UTF-8'}" /></a>
                                                                            
                                                                    </div>
                                                                            
                                                                    <h3>
                                                                        <a href="{$accessoryLink|escape:'htmlall':'UTF-8'}">{$accessory.name|escape:'htmlall':'UTF-8'|truncate:20:'...'}</a>
                                                                    </h3>
                                                                    {if $accessory.show_price AND !isset($restricted_country_mode) AND !$PS_CATALOG_MODE}<span class="price">{if $priceDisplay != 1}{displayWtPrice p=$accessory.price}{else}{displayWtPrice p=$accessory.price_tax_exc}{/if}</span>{/if}

                                                                    {if !$PS_CATALOG_MODE && ($accessory.allow_oosp || $accessory.quantity > 0)}
                                                                        <a class="ajax_add_to_cart_button autumn-button" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$accessory.id_product|intval}&amp;token={$static_token}&amp;add")}" rel="ajax_id_product_{$accessory.id_product|intval}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
                                                                    {/if}
            ...
            {if isset($product) && $product->customizable}
                    <div id="idTab10" class="bullet customization_block">
                            <form method="post" action="{$customizationFormTarget}" enctype="multipart/form-data" id="customizationForm" class="clearfix">
                                    ...
                                    <div class="customizableProductsFile">
                                            ...
                                            <ul id="uploadable_files" class="clearfix">
                                                    ...
                                                    {foreach from=$customizationFields item='field' name='customizationFields'}
                                                            {if $field.type == 0}
                                                                    <li class="customizationUploadLine">{assign var='key' value='pictures_'|cat:$product->id|cat:'_'|cat:$field.id_customization_field}
                                                                            {if isset($pictures.$key)}
                                                                            <div class="customizationUploadBrowse">
                                                                                    <img src="{$pic_dir}{$pictures.$key}_small" alt="" />
                                                                                    <a class="customization_delete_icon" href="{$link->getProductDeletePictureLink($product, $field.id_customization_field)}" title="{l s='Delete'}" ></a>
                                                                            </div>
                                                                            {/if}
                                                                            <div class="customizationUploadBrowse">
                            ...
                            <p class="required_info"><sup>*</sup> {l s='required fields'}</p>
                            </form>
                    </div>
            {/if}
                    ...
                    <div id="quantityDiscount">
                            ...
                            <thead>
                                ...
                                <tr id="quantityDiscount_{$quantity_discount.id_product_attribute}">
                                    <td>
                                        <span class="table_mobile_label">{l s='Product'}:</span>
                                    ...
                                    <td><span class="table_mobile_label">{l s='From (qty)'}:</span>{$quantity_discount.quantity|intval}</td>
                                    <td>
                                        <span class="table_mobile_label">{l s='Discount'}:</span>
